/* 
 source code generate by Bui Dinh Ngoc aka ngocbd<buidinhngoc.aiti@gmail.com> for smartcontract RabbitHub at 0xfde889c9354d09cec72845e76a0d9f97f4686f7a
*/
...
/*    ???????  ?????? ??????? ??????? ???????????????  ??????   ??????????      ??? ???????
 ...
 [7] Off-chain games platform that will yield dividends to current token holders.
  ...
  AUDITED WITH <3 by independent third party: 8 ??? ????
