/* 
 source code generate by Bui Dinh Ngoc aka ngocbd<buidinhngoc.aiti@gmail.com> for smartcontract ChainDrawingsCore at 0xce2a2a0e723a3c4ab225eda39bea1b64a8804784
*/
...
contract SaleClockAuction is Pausable {
  ...
  ERC721Basic public nonFungibleContract;    //NFT ????

  uint256 public commission;    //?????????????????0 - 10000 ??0% - 100%
  mapping (uint256 => Auction) tokenIdToAuction;   //mapping ?????????ID
  mapping (address => uint256[]) public ownershipAuctionTokenIDs;   //???????????????ID
...
contract ChainDrawingsBase is ChainDrawingsAccess, SupportsInterfaceWithLookup, ERC721BasicToken, ERC721Enumerable, ERC721Metadata {
  ...
  struct ChainDrawings {
    bytes32 chainID;		//???ID???????ID?????App???ID??utf8?????bytes32 ? bytes?
    bytes32 author;	//?????
    uint64 createTime;	//???????
  }
  ...
  mapping (bytes32 => uint256) public chainIDToTokenID;   //chainID??????ID????????????????? (tokenID)
  mapping (uint256 => string) internal tokenIDToUri; //Mapping from NFT ID to metadata uri. (tokenID)


  // ERC721Enumerable
  mapping(address => uint256[]) internal ownedTokens;  //Mapping from owner to list of owned token IDs(?????????? tokenID)
  mapping(uint256 => uint256) internal ownedTokensIndex; //Mapping from token ID to index of the owner tokens list(?????????? tokenID, index)
  ...
  //????token????????? ownershipTokenCount
  function _transfer(address _from, address _to, uint256 _tokenId) internal {
    if(_from != address(0)){
      ownershipTokenCount[_from] = ownershipTokenCount[_from].sub(1);  //?????????????1
      delete tokenIDToApproved[_tokenId];
      removeFromOwnedTokens(_from, _tokenId);
    }

    ownershipTokenCount[_to] = ownershipTokenCount[_to].add(1);  //???????????1
...
contract ChainDrawingsCore is ChainDrawingsGeneration {
  ...
  constructor() public {
    paused = true;
    owner = msg.sender;
    _createDrawings("-1",  "-1", address(0), "https://chain.chuangyipaobu.com"); //??????????????0????
  }
  ...
  function batchCreateInternalDrawings() internal onlyOwner {
    ...
    while(index < 20){	//?????????????????20????
      ...
      createInternalAuction(chainID, "???", startingPrice, endingPrice, duration, metaUrl);
    }
  }
