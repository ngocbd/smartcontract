...
contract FoMo3DLong is modularLong, Ownable, FoMo3DLongInterface {
    ...
    uint256 constant public threshould_ = 3;//??XXX?cos
    ...
    //     bool hadProp;
    //     uint256 propID;
    //     uint256 redtPRProp; //lossReductionRate???????????????
    //     uint256 incrPRProp; //Income increase rate?????
    //     uint256 team;
    //     bool first;
    //     uint256 firstCosd;//???????COS??????20% ???
    //     uint256 firstCosc;//???????COS??????20% ???
    //     uint256 redtInFirst;
    //     uint256 wonCosd;
    //     uint256 wonCosc;
    //     uint256 wonEth;
    ...
    returns(uint256)
    {
        ...
        if (_eth > 1000000000)
        {
            ...
            if(_keyType == "cosd"){
                ...
                // plyrRnds_[0][_rID].cosd      = plyrRnds_[0][_rID].cosd.add(_com);     //???
                ...
                // plyrRnds_[0][_rID].cosc      = plyrRnds_[0][_rID].cosc.add(_com);     //???
...
library F3Ddatasets {
    ...
    struct Fee {
        uint256 aff;          // % of buy in thats paid to referrer  of current round???????
