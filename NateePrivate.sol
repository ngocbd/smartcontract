/* 
 source code generate by Bui Dinh Ngoc aka ngocbd<buidinhngoc.aiti@gmail.com> for smartcontract NateePrivate at 0x67a9d6d1521e02ecfb4a4c110c673e2c027ec102
*/
...
contract MyToken is ERC20 {
     ...
     uint256 public m_nTotalSupply;  // ??????????????? private ?????????????????????????     
...
contract Ownable {
  ...
  /**
   * @dev Ownable constructor ??????????????? sender ??????? `owner` ??????????? contract 
   *
   */
   constructor() public {
    owner = msg.sender;
 ...
 // For Single Owner
  ...
  function transferOwnership(address newOwner) public onlyOwner{
    require(isContract(newOwner) == false); // ??????????????????????? contract address ?????
